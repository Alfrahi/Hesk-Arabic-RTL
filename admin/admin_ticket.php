...
<div class="main__content ticket">
                            ...
                            if ( $hesk_settings['attachments']['use'] && strlen($note['attachments']) )
                            {
                                ...
                                foreach ($att as $myatt)
                                {
                                    ...
                                    if ($can_del_notes || $note['who'] == $_SESSION['id'])
                                    {
                                        ...
                                        {
                                            echo '<a class="tooltip" data-ztt_vertical_offset="0" style="margin-left: 8px;" href="admin_ticket.php?delnote='.$note['id'].'&amp;track='.$trackingID.'&amp;Refresh='.mt_rand(10000,99999).'&amp;token='.hesk_token_echo(0).'" onclick="return hesk_confirmExecute(\''.hesk_makeJsString($hesklang['pda']).'\');" title="'.$hesklang['dela'].'">
                                        ...
                                        {
                                            echo '<a class="tooltip" data-ztt_vertical_offset="0" style="margin-left: 8px;" href="admin_ticket.php?delatt='.$att_id.'&amp;note='.$note['id'].'&amp;track='.$trackingID.'&amp;Refresh='.mt_rand(10000,99999).'&amp;token='.hesk_token_echo(0).'" onclick="return hesk_confirmExecute(\''.hesk_makeJsString($hesklang['pda']).'\');" title="'.$hesklang['dela'].'">
    ...
    <div class="ticket__params">
        <section class="params--bar" style="padding-right: 0">
        ...
        <section class="params--block details accordion visible">
            ...
            <div class="accordion-body" style="display:block">
                ...
                <div class="row">
                    ...
                    if ($can_due_date)
                    {
                        $hesk_settings['datepicker']['#new-due-date']['position'] = 'right bottom';
                        ?>
                        <div class="value">
                                    ...
                                    <section class="param calendar">
                                        <div class="calendar--button" id="due-date-button">
                                            ...
                                            <input name="new-due-date" id="new-due-date"
                                                   data-datepicker-position="right top"
...
function hesk_listAttachments($attachments='', $reply=0, $white=1)
{
	...
	foreach ($att as $myatt)
	{
        ...
        {
        	echo '<a class="tooltip" data-ztt_vertical_offset="0" style="margin-left: 8px;" title="'.$hesklang['dela'].'" href="admin_ticket.php?delatt='.$att_id.'&amp;reply='.$reply.'&amp;track='.$trackingID.'&amp;Refresh='.mt_rand(10000,99999).'&amp;token='.hesk_token_echo(0).'" onclick="return hesk_confirmExecute(\''.hesk_makeJsString($hesklang['pda']).'\');">
		...
		<a title="'.$hesklang['dnl'].' '.$att_name.'" href="../download_attachment.php?att_id='.$att_id.'&amp;track='.$trackingID.'">
            <svg class="icon icon-attach" style="width: 16px; height: 16px; margin-left: 0px; vertical-align: text-bottom;">
                <use xlink:href="'. HESK_PATH .'img/sprite.svg#icon-attach"></use>
            </svg>
        </a>
...
function hesk_getAdminButtons($isReply=0,$white=1)
{
    ...
    if ($can_edit)
    {
        ...
        if ($isReply) {
            $buttons['more']['edit'] = '
        <a id="editreply'.$reply['id'].'" href="edit_post.php?track='.$trackingID.$tmp.'" title="'.$hesklang['btn_edit'].'" style="margin-left: 15px">
            <svg class="icon icon-edit-ticket">
    ...
    <form method="post" class="form" action="admin_reply_ticket.php" enctype="multipart/form-data" name="form1" onsubmit="force_stop();<?php if ($hesk_settings['staff_ticket_formatting'] != 2): ?>clearTimeout(typingTimer);<?php endif; ?>return true;">
        ...
        if ($hesk_settings['time_worked']) {
            ...
            <section class="block--timer">
                ...
                <div class="form-group short" style="margin-right: 8px; margin-bottom: 0">
